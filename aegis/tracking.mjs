export function countFingers(landmarks, width = 1, height = 1) {
  ...
  if (palm < 1e-8) return 0;
  const margin = palm * 0.08;
  return (
    Number(distance(4, 17) > distance(3, 17) + margin) +
    [8, 12, 16, 20].filter((t) => distance(t, 0) > distance(t - 2, 0) + margin)
      .length
  );
}
export class StableCount {
  constructor(window = 7) {
    this.window = window;
    this.values = [];
    this.hands = null;
  }
