...
export async function loadDetectors(includePose){
 let hand=null,pose=null;
 try{
  const {FilesetResolver,HandLandmarker,PoseLandmarker}=await import(BASE+'/vision_bundle.mjs');
  const files=await FilesetResolver.forVisionTasks(BASE+'/wasm');
  hand=await HandLandmarker.createFromOptions(files,{
   baseOptions:{modelAssetPath:HAND,delegate:'CPU'},runningMode:'VIDEO',numHands:2,
   minHandDetectionConfidence:.65,minHandPresenceConfidence:.65,minTrackingConfidence:.65
  });
  if(includePose)pose=await PoseLandmarker.createFromOptions(files,{
   baseOptions:{modelAssetPath:POSE,delegate:'CPU'},runningMode:'VIDEO',numPoses:1,
  ...
  return{
   ...
   close(){try{hand?.close();}finally{pose?.close();}}
  };
 }catch(error){try{hand?.close();}finally{pose?.close();}throw error;}
}
