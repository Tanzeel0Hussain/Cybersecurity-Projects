...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Analyze authorized SSH/authentication logs for failed login patterns"
    )
    parser.add_argument(
        "logfile",
        nargs="?",
        default="sample_auth.log",
        help="Path to auth log (default: sample_auth.log)",
    )
    parser.add_argument(
        "--output",
    ...
    parser.add_argument(
        "--json-output",
        help="Optional JSON report output file, e.g. report.json",
    )
    parser.add_argument(
        "--threshold",
        type=int,
        default=FAILED_THRESHOLD,
        help="Failed-attempt threshold for suspicious activity",
    )
...
def generate_report(
    ...
    with open(
        ...
        if not ip_counter:
            file.write(
                "No failed-login IP addresses detected.\n"
            )
        ...
        if not user_counter:
            file.write(
                "No targeted usernames detected.\n"
            )

        for user, count in user_counter.most_common():
            file.write(
                f"{user} -> {count} attempts\n"
            )
